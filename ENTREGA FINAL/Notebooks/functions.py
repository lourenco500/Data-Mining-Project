...
def plot_profiling_barplots(
    ...
    """
    Plots a stacked bar chart for a categorical variable profile.
    To be used inside subplots (does NOT create figures or legends).
    """
...
def plot_profiling_heatmaps(
    ...
    """
    Plot side-by-side heatmaps for multiple groups of one-hot encoded features.
    ...
    group_keys : list of str
        Feature groups to plot (e.g. ['City', 'Province or State']).
    figsize : tuple
        Overall figure size.
    cmap : str
        Seaborn colormap.
...
def plot_profiling_radar(
    ...
    """
    Plot radar charts for one or more groups of one-hot encoded categorical
    features, sharing a single legend.

    Parameters
    ----------
    group_keys : list of str
        Categorical feature groups to plot (e.g. ['Income_Bins', 'Marital Status']).
    profile_df : pd.DataFrame
        Cluster-level profiling dataframe (rows = clusters, columns = OHE features).
    groups : dict
        Dictionary mapping feature group names to lists of corresponding
        one-hot encoded columns.
    alpha : float, optional
        Transparency level for the filled radar areas.
    """
